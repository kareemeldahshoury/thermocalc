...
200: {  
     ...
     60:   [0.0010084, 247.75,  267.92, 0.8028],
    ...
    300:   [0.0013611, 1307.2, 1334.3, 3.2090],
    320:   [0.0014046, 1416.6, 1445.5, 3.3996],
...
300: { 
    ...
    300:   [0.0013322, 1288.9,  1328.9, 3.1753],
    320:   [0.0014042, 1391.7,  1433.7, 3.3558],
...
500: { 
    ...
    240:   [0.0011708, 990.55, 1049.10,   2.6156],
    260:   [0.0012044, 1078.20, 1138.40,  2.7864],
    280:   [0.0012430, 1167.70, 1229.90,  2.9547],
    300:   [0.0012879, 1259.60, 1324.00,  3.1218],
    320:   [0.0013409, 1354.30, 1421.40,  3.2888],
    340:   [0.0014049, 1452.90, 1523.10,  3.4575],
    360:   [0.0014848, 1565.60, 1630.70,  3.6375],
    380:   [0.0015884, 1667.10, 1746.50,  3.8102],
}
...
class CompLiqWaterCalculation:
    def __init__(self, inputs: Dict[str, str] | None):
        self.comp_tables: Dict[int, Dict[float, List[float]]] = compLiqWater_data  
    ...
    def get_properties(self, pressure: float, temp: float):
        if pressure not in self.comp_tables:
            raise ValueError(f"Pressure {pressure} bar not available in Dataset.")

        table = self.comp_tables[pressure]
        return self._interp_within_table(table, temp)
    ...
    def _interp_within_table(self, table: Dict[float, List[float]], temp: float):
        ...
        if lower is None or higher is None:
            raise ValueError(f"Temperature {temp}°C out of range for pressure {self.pressure} bar.")
