...
ideal_air = {
    ...
    770: [800.03, 43.35, 576.12, 51.64, 2.69013],
    780: [821.95, 47.75, 592.30, 48.08, 2.71787],
    790: [843.98, 52.59, 608.59, 44.84, 2.74504],
    800: [866.08, 57.60, 624.95, 41.85, 2.77170],
    810: [888.27, 63.09, 641.40, 39.12, 2.79783],
    820: [910.56, 68.98, 657.95, 36.61, 2.82344],
    ...
    1360: [1467.49, 399.1, 1077.10,  9.780, 3.32728],
    ...
    1700: [1880.10, 1025.0, 1392.7, 4.761, 3.59790],
    1750: [1941.60, 1161.0, 1439.8, 4.328, 3.63360],
    1800: [2003.30, 1310.0, 1487.2, 3.994, 3.66840],
    1850: [2065.30, 1475.0, 1534.9, 3.601, 3.70230],
    1900: [2127.40, 1655.0, 1582.6, 3.295, 3.73540],
    1950: [2189.70, 1852.0, 1630.6, 3.022, 3.76770],
    2000: [2252.10, 2068.0, 1678.7, 2.776, 3.79940],
    2050: [2314.60, 2303.0, 1726.8, 2.555, 3.83030],
    2100: [2377.70, 2559.0, 1775.3, 2.356, 3.86050],
    2150: [2440.30, 2837.0, 1823.8, 2.175, 3.89010],
    2200: [2503.20, 3138.0, 1872.4, 2.012, 3.91910],
    2250: [2566.40, 3464.0, 1921.3, 1.864, 3.94740]
}

def interpolate(val1, val2, x1, x2, x):
    return val1 + (val2 - val1) * (x - x1) / (x2 - x1)

class IdealAirProperties:
    def __init__(self, inputs: Dict[str, str]):
        self.inputs = inputs
        self.T = float(inputs.get("temperature", 0))
        self.temps = sorted(ideal_air.keys())

    def calculate(self) -> Dict[str, float]:
        if not (self.temps[0] <= self.T <= self.temps[-1]):
            raise ValueError(f"Temperature {self.T} K out of range.")

        if self.T in ideal_air:
            h, Pr, u, vr, s = ideal_air[self.T]
        else:
            ...
            Pr = interpolate(v1[1], v2[1], T1, T2, self.T)
            u = interpolate(v1[2], v2[2], T1, T2, self.T)
            vr = interpolate(v1[3], v2[3], T1, T2, self.T)
